...
import { GEN_CONFIG_RELATIVE } from '../project/config';
 ...
 *
 * VS Code `contributes.configuration` сейчас только `gen.chatViewLocation`
 * (синхронизируется из effective settings) - не отдельный файл-слой.
 * UI Settings не ломаем: слои аддитивны; project перекрывает user и UI
 * только по ключам, явно заданным в JSON; admin нельзя обойти.
 */
...
const EMPTY_PARSED: ParsedFileConfig = { settings: {} };
...
export async function reloadConfigLayers(): Promise<ConfigLayersSnapshot> {
	...
	snapshot = {
		user: userRaw !== undefined ? parseFileConfig(userRaw) : { settings: {} },
		project: projectRaw !== undefined ? parseFileConfig(projectRaw) : { settings: {} },
		userPath,
...
export function initConfigLayers(context: vscode.ExtensionContext): void {
	...
	const startProjectWatcher = (): void => {
		...
		const watcher = vscode.workspace.createFileSystemWatcher(
			new vscode.RelativePattern(folder, GEN_CONFIG_RELATIVE),
		);
