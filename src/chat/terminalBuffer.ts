...
/**
 * Подписка на вывод терминалов через стабильный API
 * `onDidEndTerminalShellExecution` + `execution.read()`.
 *
 * Не используем proposed `onDidWriteTerminalData`: без `enabledApiProposals`
 * VS Code падает при активации расширения.
...
export function getTerminalBuffers(): Array<{ name: string; text: string }> {
