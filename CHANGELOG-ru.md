...
- Composer: chips контекста для `@file` / `@folder` / `@codebase`
- Сворачиваемая карточка плана над полем ввода
- Правка сообщения пользователя в чате; история после правки обрезается, ход запускается заново
- Accept / Reject по хункам в карточке tool-diff в чате
- Настройки: отключить запись `.gen/plan.md` (`planWriteToFile`) и скрыть блок плана в чате (`showPlanCard`)
